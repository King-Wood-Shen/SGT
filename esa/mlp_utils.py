        ...
        if num_layers == 1:
            ...
            for i in range(num_layers):
                if i == 0:
                    ...
                    self.mlp.append(nn.LayerNorm(inter_dim))
                    # self.mlp.append(neuron.LIFnode(tau=2.0, detach_reset=True, backend="torch"))
                    self.mlp.append(nn.Linear(inter_dim, out_dim))

                if dropout_p > 0:
                    self.mlp.append(nn.Dropout(p=dropout_p))
            self.mlp.append(nn.LayerNorm(out_dim))
...
class GatedMLPSingle(nn.Module):
    ...
    def forward(self, x):
        if self.use_ln:
            # print(111111)
            y = self.ln(self.fc1(self.lif1(x)))
        else:
            y = self.fc1(self.lif1(x))
