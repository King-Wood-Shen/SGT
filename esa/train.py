...
def main():
    ...
    parser.add_argument("--no-dataset-one-hot", dest='dataset_one_hot', action='store_false')
    parser.add_argument("--dataset-target-name", type=str,default='None')

    # Learning hyperparameters
    parser.add_argument("--lr", type=float,default='0.0001')
    parser.add_argument("--batch-size", type=int, default=128)
    parser.add_argument("--norm-type", type=str, choices=["BN", "LN"],default='LN')
...
# 兼容Python 3.8：默认启用bfloat16，--no-use-bfloat16可禁用
    parser.add_argument("--use-bfloat16", default=True, help="启用bfloat16混合精度训练（默认启用）",type=bool)
    # parser.add_argument("--no-use-bfloat16", dest='use_bfloat16', action='store_false', help="禁用bfloat16混合精度训练")

    # Node/graph dimensions
    parser.add_argument("--graph-dim", type=int,default=512)

    # ESA arguments
    parser.add_argument("--xformers-or-torch-attn", type=str, choices=["xformers", "torch"],default='xformers')
    parser.add_argument("--apply-attention-on", type=str, choices=["node", "edge"], default="edge")
    parser.add_argument("--layer-types", type=str, nargs="+",default=['S','M','M','S','P'])
    parser.add_argument("--hidden-dims", type=int, nargs="+",default=[256,256,256,256,256])
    parser.add_argument("--num-heads", type=int, nargs="+",default=[4,4,4,4,4])
    parser.add_argument("--pre-or-post", type=str, choices=["pre", "post"], default="post")
    parser.add_argument("--sab-dropout", type=float, default=0.1)
    parser.add_argument("--mab-dropout", type=float, default=0.1)
    parser.add_argument("--pma-dropout", type=float, default=0.1)
    parser.add_argument("--attn-residual-dropout", type=float, default=0.1)
    parser.add_argument("--pma-residual-dropout", type=float, default=0.1)
    parser.add_argument("--pos-enc", type=str)
    parser.add_argument("--T",type=int,default=1)
    # MLP arguments
# 兼容 Python 3.8：默认启用 MLPs，--no-use-mlps 可禁用
    parser.add_argument("--use-mlps", default=True, action='store_true', help="启用注意力块后的 MLPs（默认启用）")
    # parser.add_argument("--no-use-mlps", dest='use_mlps', action='store_false', help="禁用注意力块后的 MLPs")
    ...
    if dataset != "ocp":
        ...
        # Graph-level branch
            ...
            print(dataset,dataset_download_dir,dataset_one_hot,target_name,posenc)
    ...
    model_args = {**model_args , ** dict(
        ...
        use_mlp_ln=use_mlp_ln, mlp_dropout=mlp_dropout,device=args.cuda,T=args.T
    )}

    if dataset != "ocp":
        # 兼容 Python 3.8：用 update() 原地合并字典，效果与 |= 一致
        model_args.update(dict(is_node_task=check_is_node_level_dataset(dataset)))
    ...
    # 兼容 Python 3.8：通过字典解包合并两个字典，生成新字典赋值给 trainer_args
