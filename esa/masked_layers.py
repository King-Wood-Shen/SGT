...
def get_adj_mask_from_edge_index_node(
    edge_index, batch_size, max_items, batch_mapping, xformers_or_torch_attn, use_bfloat16=True, device="cuda:2"
):
...
def get_first_unique_index(t,device):
    ...
    zero = torch.tensor([0], device=torch.device(device))
...
def generate_consecutive_tensor(input_tensor, final,device):
    # Calculate the length of each segment
    lengths = input_tensor[1:] - input_tensor[:-1]

    # Append the final length
    lengths = torch.cat((lengths, torch.tensor([final - input_tensor[-1]], device=torch.device(device))))

    # Create ranges for each segment
    ranges = [torch.arange(0, length, device=torch.device(device)) for length in lengths]
...
def get_adj_mask_from_edge_index_edge(
    ...
    if xformers_or_torch_attn in ["torch"]:
        ...
        empty_mask_fill_value = 0
        mask_dtype = torch.bfloat16 if use_bfloat16 else torch.float32
        edge_mask_fill_value = 1
    ...
    edge_batch_index_to_original_index = generate_consecutive_tensor(
        get_first_unique_index(edge_batch_mapping,device=device), edge_batch_mapping.shape[0],device=device
    )
...
class ESA(nn.Module):
    ...
    def forward(self, X, edge_index, batch_mapping, num_max_items,device='cuda:2'):
        ...
        if hasattr(self, "decoder"):
            ...
            out = enc
        # print(out.shape)
        return F.mish(self.decoder_linear(self.lif(out)))
