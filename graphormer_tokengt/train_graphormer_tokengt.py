...
parser.add_argument("--dataset_download_dir", type=str,default='/data01/zjx/esa')
parser.add_argument("--dataset-one-hot", default=False, action='store_true')
parser.add_argument("--dataset-target-name", type=str,default='None')
parser.add_argument("--batch_size", type=int, default=32)
parser.add_argument("--out_dir", type=str,default='/data01/zjx/esa/output')
parser.add_argument("--name", type=str)
parser.add_argument("--wandb_project_name", type=str)
parser.add_argument("--architecture", type=str, choices=["graphormer", "tokengt"],default='graphormer')
...
parser.add_argument("--ckpt_path",default='/data01/zjx/esa', type=str)
...
from spikingjelly.activation_based import neuron, layer, functional  # 导入functional模块
from graphormer_tokengt.graphormer.modelling_graphormer import GraphormerForGraphClassification
...
from graphormer_tokengt.tokengt.modeling_tokengt import TokenGTForGraphClassification
...
torch.backends.cudnn.benchmark = False     # 禁用自动选择最优卷积算法
torch.use_deterministic_algorithms(True)   # 强制 PyTorch 所有算子使用确定性实现
...
def main():
    ...
    if args.architecture == "graphormer":
        ...
        model = GraphormerForGraphClassification(config=config)
        print(model)
    ...
    if "struct" in args.dataset_name:
        ...
        greater_is_better = False
    output_dir_with_seed = f"{args.out_dir}_seed_{args.seed}"
    # 转换为 Path 对象，方便后续操作
    output_dir = Path(output_dir_with_seed)
