...
class MultiheadAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.head_lif=neuron.LIFNode(tau=2.0, detach_reset=True, backend="torch")
        ...
        self.out_norm=nn.LayerNorm(config.embedding_dim)
        self.out_lif=neuron.LIFNode(tau=2.0, detach_reset=True, backend="torch")
        self.q_norm=nn.LayerNorm(config.embedding_dim)
        self.k_norm=nn.LayerNorm(config.embedding_dim)
        self.v_norm=nn.LayerNorm(config.embedding_dim)
    ...
    def forward(
        ...
        q = q.view(bsz, src_len, self.num_heads, self.head_dim).transpose(1, 2)  # [B, H, T, head_dim]
        k = k.view(bsz, src_len, self.num_heads, self.head_dim).transpose(1, 2)  # [B, H, T, head_dim]
        v = v.view(bsz, src_len, self.num_heads, self.head_dim).transpose(1, 2)  # [B, H, T, head_dim]

        # 3. 打包 QKV 并去填充（保留 unpad_input）
        qkv = torch.stack([q, k, v], dim=2)  # [B, H, 3, T, head_dim]
        qkv = qkv.transpose(1, 3)  # [B, T, 3, H, head_dim]（适配 unpad_input 输入格式）

        # 调用 unpad_input 处理变长序列（保留原去填充逻辑）
        qkv_unpad, indices, cu_q_lens, max_s, _ = unpad_input(qkv, ~key_padding_mask)
        # 调整形状：[total_tokens, 3, H, head_dim]（与原 flash_attn 输入格式一致）
        qkv_unpad = qkv_unpad.view(-1, 3, self.num_heads, self.head_dim)

        # 4. 手动实现注意力计算（替代 flash_attn_varlen_qkvpacked_func）
        total_tokens, _, num_heads, head_dim = qkv_unpad.shape
        batch_size = bsz

        # 拆分 Q、K、V（从去填充的打包张量中）
        q_unpad = qkv_unpad[:, 0]  # [total_tokens, H, head_dim]
        k_unpad = qkv_unpad[:, 1]  # [total_tokens, H, head_dim]
        v_unpad = qkv_unpad[:, 2]  # [total_tokens, H, head_dim]

        # 按样本拆分并计算注意力
        outputs_unpad = []
        for i in range(batch_size):
            # 从累积长度获取当前样本的 token 范围
            start = cu_q_lens[i].item()
            end = cu_q_lens[i + 1].item()
            seq_len = end - start  # 当前样本有效长度

            if seq_len == 0:
                # 空序列直接返回零
                outputs_unpad.append(torch.zeros(0, num_heads, head_dim, device=q_unpad.device))
                continue

            # 提取当前样本的 Q、K、V
            q_i = q_unpad[start:end]  # [seq_len, H, head_dim]
            k_i = k_unpad[start:end]  # [seq_len, H, head_dim]
            v_i = v_unpad[start:end]  # [seq_len, H, head_dim]

            # 转置为 [H, seq_len, head_dim] 便于矩阵计算
            q_i = q_i.transpose(0, 1)  # [H, seq_len, head_dim]
            k_i = k_i.transpose(0, 1)  # [H, seq_len, head_dim]
            v_i = v_i.transpose(0, 1)  # [H, seq_len, head_dim]

            # 计算注意力分数 (Q*K^T) 并缩放
            attn_scores = torch.matmul(q_i, k_i.transpose(-2, -1)) * self.scaling  # [H, seq_len, seq_len]

            # 应用 softmax 和 dropout
            # attn_probs = F.softmax(attn_scores, dim=-1)
            attn_probs = F.dropout(attn_scores, p=self.attention_dropout if self.training else 0.0, training=self.training)

            # 注意力加权求和（与 V 相乘）
            output_i = torch.matmul(attn_probs, v_i)  # [H, seq_len, head_dim]

            # 转回 [seq_len, H, head_dim] 并添加到结果
            outputs_unpad.append(output_i.transpose(0, 1))

        # 拼接所有样本的去填充结果
        output_unpad = torch.cat(outputs_unpad, dim=0)  # [total_tokens, H, head_dim]

        # 5. 调用 pad_input 恢复填充（保留原填充逻辑）
        output = pad_input(output_unpad, indices, batch_size, src_len)  # [B, T, H, head_dim]

        # 6. 维度调整并投影输出
        output = output.reshape(bsz, tgt_len, embedding_dim)  # 合并多头 [B, T, D]
        output = output.transpose(0, 1)  # [T, B, D]

        attn = self.out_proj(self.out_lif(self.out_norm(output)))
...
class FeedForward(nn.Module):
    def __init__(self, config):
        ...
        self.lif1=neuron.LIFNode(tau=2.0, detach_reset=True, backend="torch")
        self.activation_fn = ACT2FN[config.activation_fn]
        self.activation_dropout_module = torch.nn.Dropout(p=config.activation_dropout)
        self.lif2=neuron.LIFNode(tau=2.0, detach_reset=True, backend="torch")
        self.norm2=nn.LayerNorm(config.ffn_embedding_dim)
    ...
    def forward(self, x):
        x = self.activation_fn(self.fc1(self.lif1(x)))
        x = self.activation_dropout_module(x)

        x = self.fc2(self.lif2(self.norm2(x)))
...
class TokenGTForGraphClassification(TokenGTPreTrainedModel):
    ...
    def forward(
        ...
        for i in range(self.T):
            ...
            prediction_result.append(head_outputs)
            # print(torch.flatten(head_outputs).shape)

        head_outputs = torch.stack(prediction_result).mean(dim=0)
